...
export function DetalleEnvioPage({ navigate, params, toast }: Props) {
  ...
  const docs = [
    { name: 'Carta de porte', ext: 'PDF', date: '7 jun 2026', status: 'Disponible' },
    { name: 'Factura CFDI 4.0', ext: 'XML', date: '7 jun 2026', status: 'Disponible' },
    { name: 'Factura CFDI 4.0', ext: 'PDF', date: '7 jun 2026', status: 'Disponible' },
    { name: 'Seguro de carga', ext: 'PDF', date: '7 jun 2026', status: 'Disponible' },
    { name: 'Comprobante de entrega', ext: 'PDF', date: 'Pendiente', status: 'Pendiente' },
  ]
    ...
    <div>
      ...
      <div className="page-head">
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          ...
          <span className="mono" style={{ fontSize: 13.5, color: 'var(--text-faint)' }}>{shipment.id}</span>
        </div>

        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
          <div>
            <h1 className="page-title" style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span className="mono" style={{ fontSize: 22, color: 'var(--primary)', fontWeight: 700 }}>{shipment.id}</span>
            </h1>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            {shipment.status === 'transit' && (
              <Button variant="secondary" icon="mapPin" onClick={() => navigate('rastreo', { id: shipment.id })}>Rastrear</Button>
            )}
            {shipment.status === 'waiting' && (
              <Button variant="success" icon="dollar" onClick={() => navigate('pago', { id: shipment.id })}>Pagar ahora</Button>
            )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.5fr) minmax(0,1fr)', gap: 18, alignItems: 'start' }} className="track-cols">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          {/* Map */}
          <StaticRouteMap origin={`${shipment.origin} (${shipment.oCode})`} dest={`${shipment.dest} (${shipment.dCode})`} />
          ...
          <Card>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Placa ABC-1234 · Conductor: Juan García</div>
              </div>
            </div>
          </Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <Card>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
              <span style={{ fontSize: 13.5, color: 'var(--text-muted)' }}>Anticipo (50%)</span>
              <span className="mono tnum" style={{ fontWeight: 600, color: 'var(--green-600)' }}>{fmtUSD(shipment.paid)}</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 12 }}>
              <span style={{ fontSize: 13.5, color: 'var(--text-muted)' }}>Restante al entregar</span>
            ...
            <div style={{ height: 6, background: 'var(--gray-100)', borderRadius: 999 }}>
              <div style={{ height: 6, background: 'var(--green-500)', borderRadius: 999, width: `${(shipment.paid / shipment.price) * 100}%`, transition: 'width 0.5s' }} />
            </div>
            {shipment.status === 'waiting' && (
