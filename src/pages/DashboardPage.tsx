...
import type { Route, NavParams, User, Shipment } from '../types'
...
function KpiCard({ icon, iconBg, iconColor, label, value, unit, delta, deltaUp }: KpiCardProps) {
  return (
    <Card hover style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        {delta && (
          <span className="badge badge-soft" style={{ '--bg': deltaUp ? 'var(--green-50)' : 'var(--red-50)', '--fg': deltaUp ? 'var(--green-600)' : 'var(--red-500)', fontSize: 11.5 } as React.CSSProperties}>
            <Icon name="trendingUp" size={12} style={{ transform: deltaUp ? 'none' : 'scaleY(-1)' }} />{delta}
          </span>
        )}
      </div>
      <div>
...
function MiniChart() {
  ...
  const ys = (v: number) => H - pad - ((v - 0) / (max - 0)) * (H - pad * 2)
  ...
  return (
    <Card>
      <div className="section-head" style={{ marginBottom: 4 }}>
        <div>
          ...
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 2 }}>
            Total <strong className="tnum" style={{ color: 'var(--text-strong)' }}>115</strong> envíos · <span style={{ color: 'var(--green-600)', fontWeight: 600 }}>+18% vs semestre anterior</span>
          </div>
        </div>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', marginTop: 8 }}>
        ...
        {[0, 10, 20, 30].map(g => (
          <g key={g}>
            <line x1={pad} x2={W - pad} y1={ys(g)} y2={ys(g)} stroke="var(--border-soft)" strokeWidth="1" />
            <text x={0} y={ys(g) + 4} fontSize="11" fill="var(--text-faint)">{g}</text>
          </g>
        ))}
        ...
        {data.map((v, i) => (
          <g key={i} onMouseEnter={() => setHover(i)} style={{ cursor: 'pointer' }}>
            ...
            {hover === i && (
              <g>
                <rect x={xs(i) - 20} y={ys(v) - 34} width="40" height="24" rx="6" fill="var(--gray-900)" />
                <text x={xs(i)} y={ys(v) - 18} fontSize="12" fill="#fff" textAnchor="middle" fontWeight="700">{v}</text>
              </g>
            )}
          </g>
        ))}
...
function ShipmentRow({ s, navigate, onPay }: { s: Shipment; navigate: (r: Route, p?: NavParams | null) => void; onPay: () => void }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 4px' }}>
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 5, fontSize: 13.5, color: 'var(--text)' }}>
          <span style={{ fontWeight: 600 }}>{s.oCode}</span>
          <Icon name="arrowRight" size={14} style={{ color: 'var(--text-faint)' }} />
          <span style={{ fontWeight: 600 }}>{s.dCode}</span>
          <span style={{ color: 'var(--gray-300)' }}>·</span>
          <span style={{ color: 'var(--text-muted)' }}>{s.origin} → {s.dest}</span>
        </div>
        <div style={{ fontSize: 12.5, color: 'var(--text-faint)', marginTop: 3 }}>
          {s.status === 'transit' ? `Salida: ${s.depart} · ETA: ${s.eta}`
            : s.status === 'delivered' ? `Entregado: ${s.eta}`
            : `Llegada: ${s.eta} · Esperando pago final`}
        </div>
      </div>
...
function ActionItem({ color, icon, title, code, meta, btn, btnVariant, onClick }: {
  color: string; icon: string; title: string; code: string; meta: string; btn: string; btnVariant: string; onClick: () => void
}) {
  return (
    <div style={{ display: 'flex', gap: 12, padding: 12, borderRadius: 12, border: '1px solid var(--border-soft)', background: 'var(--gray-50)' }}>
      <span style={{ width: 34, height: 34, borderRadius: 9, background: '#fff', color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: 'var(--sh-sm)' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ marginTop: 10 }}>
          <Button size="sm" variant={btnVariant as any} onClick={onClick}>{btn}</Button>
        </div>
      </div>
    </div>
  )
...
function NewsItem({ tag, tagColor, tagBg, text }: { tag: string; tagColor: string; tagBg: string; text: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
      <span className="badge badge-soft" style={{ '--bg': tagBg, '--fg': tagColor, alignSelf: 'flex-start', fontSize: 11 } as React.CSSProperties}>{tag}</span>
...
export function DashboardPage({ navigate, user, onPay }: DashboardProps) {
      ...
      <div className="page-head" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        <div>
          ...
          <p className="page-sub">
            Lunes, 7 de junio 2026 · Tienes <strong style={{ color: 'var(--text)' }}>3 envíos activos</strong> y <strong style={{ color: 'var(--orange-600)' }}>1 pago pendiente</strong>.
          </p>
        </div>
            ...
            <div style={{ padding: '0 20px 8px' }}>
              {recent.map((s, i) => (
                <div key={s.id}>
                  ...
                  <ShipmentRow s={s} navigate={navigate} onPay={() => onPay(s.id)} />
                </div>
              ))}
            </div>
          </Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <Card pad={false}>
            <div style={{ padding: '18px 20px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div className="section-title">Noticias & avisos</div>
              <span style={{ color: 'var(--primary)' }}><Icon name="bell" size={18} /></span>
            </div>
